...
# В filter_logic.py, замените существующую функцию update_metrics на:

def update_metrics(decision, duration_ms):
    """Обновление метрик качества"""
    total_requests.inc()
    analysis_duration.observe(duration_ms)
    if decision == "BLOCKED":
        blocked_by_blacklist.inc()  # или создайте отдельный counter для блокировок


def check_url(url):
    """проверка URL по черному списку и списку коротких ссылок"""
    if not url:
        return True, None
...
def check_content(text):
    """проверка контента на наличие подозрительных паттернов"""
    if not text:
        return True, None
...
def add_log(url, content, decision, reason, user_ip="127.0.0.1"):
...
def get_stats():
    """возвращает статистику разрешенных/заблокированных проверок"""
